...
- DNS en mode bridge : Utilisation du DNS pour le LAN comme pour le VPN, on utilise donc l'IP hôte à la place de l'IP docker  
    ...
    - ajout `listeningMode = "ALL"` car le LAN `192.168.1.0/24` n'est pas le réseau interne docker [cf doc](https://docs.pi-hole.net/ftldns/configfile/#listeningmode)
    - définir `dns.reply.host.IPv4 : 192.168.1.2`sur l'IP hôte pour que pihole renvoie cette IP lors d'une requpete DNS vers son hostname `dns.ddignac.fr` 